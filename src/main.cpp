...
static void RecordCallbackTiming(uint32_t elapsed) {
...
static void UpdateAmp(AppState &state) {
    ...
    if (!bypass) {
        ...
        state.model_ready = audio.Init(state.selected_amp);
    }
...
static void ReportStatus(AppState &state) {
    ...
    if (now - state.last_report_ms < 1000)
        return;
