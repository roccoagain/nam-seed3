...
void AudioCallback(daisy::AudioHandle::InputBuffer in, daisy::AudioHandle::OutputBuffer out, size_t size) {
    ...
    while (elapsed > previous && !max_callback_us.compare_exchange_weak(previous, elapsed, std::memory_order_relaxed)) {
    }
    if (elapsed >= 1000)
...
int main() {
    ...
    seed.StartLog(false);
    seed.usb_handle.SetReceiveCallback(SerialReceive, daisy::UsbHandle::FS_INTERNAL);
    ...
    while (true) {
        ...
        if (now - last_report >= 1000) {
            ...
            seed.PrintLine("NAM A2-Lite: amp=%s ready=%u bypass=%u callbacks=%lu "
                           ...
                           AmpName(selected), static_cast<unsigned>(model_ready), static_cast<unsigned>(bypass || !model_ready), static_cast<unsigned long>(audio_callbacks.exchange(0)), static_cast<unsigned long>(max_callback_us.exchange(0)), static_cast<unsigned long>(overruns.exchange(0)));
        }
        seed.DelayMs(1);
    }
}
