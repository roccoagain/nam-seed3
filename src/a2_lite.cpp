...
namespace {
constexpr int kKernels[] = {6, 6, 6,  6,  6, 6, 6, 6, 6, 6, 6, 6,
                            6, 6, 15, 15, 6, 6, 6, 6, 6, 6, 6};
constexpr int kDilations[] = {1, 3, 7,  17, 41,  101, 239, 1,
                              3, 7, 17, 41, 101, 239, 1,   13,
                              1, 3, 7,  17, 41,  101, 239};
} // namespace
...
float A2Lite::Sample(float input) {
  ...
  for (auto &layer : layers_) {
    ...
    for (int tap = 0; tap < layer.kernel; ++tap, w += 9) {
      const uint32_t pos =
          (layer.position -
           static_cast<uint32_t>((layer.kernel - 1 - tap) * layer.dilation)) &
          layer.mask;
    ...
    for (int c = 0; c < 3; ++c)
      x[c] += tail[15 + c] + tail[6 + c * 3] * z[0] + tail[7 + c * 3] * z[1] +
              tail[8 + c * 3] * z[2];
