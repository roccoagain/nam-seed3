...
int main() {
  for (int id = 1; id <= 3; ++id) {
    ...
    for (int offset = 0; offset < kTestSamples;) {
      ...
      for (int i = 0; i < frames; ++i) input[i] = TestInput(offset + i);
    ...
    std::cout << AmpName(static_cast<AmpId>(id)) << ": max error=" << maximum
              << " ESR=" << error / energy << std::endl;
    ...
    for (int i = 0; i < 48; ++i) assert(std::fabs(output[i] - reference[i]) < 1e-4);
  }
}
